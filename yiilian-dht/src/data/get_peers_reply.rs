...
use anyhow::anyhow;
use bytes::Bytes;
use yiilian_core::{frame::Frame, util::{bytes_to_sockaddr, sockaddr_to_bytes}, YiiLianError, extract_bencode_field_from_map};
    ...
    fn try_from(frame: Frame) -> Result<Self, Self::Error> {
        ...
        if !frame.verify_items(&[("y", "r")]) {
            return Err(YiiLianError::FrameParse(anyhow!(
                "not valid frame for GetPeersFeedback, frame: {frame}"
            )));
        }
        ...
        let token = extract_bencode_field_from_map!(r, "token", frame)?;

        let nodes = if let Some(node_bytes) = r.get("nodes".as_bytes()) {
        ...
        let values = if let Some(value_bytes) = r.get("values".as_bytes()) {
...
impl From<&GetPeersReply> for Frame {
    fn from(value: &GetPeersReply) -> Self {
        ...
        r.insert("nodes".into(), merge_node_bytes!(&value.nodes, ID_SIZE).into());
