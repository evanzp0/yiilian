    ...
    {$map: ident, $field: expr, $frame: ident} => {
        {
            ...
            if let Some(value) = $map.get($field.as_bytes()) {
                ...
                Err(YiiLianError::FrameParse(anyhow::anyhow!(
                    "feedback frame is error, frame: {}",
                    $frame.to_string()
                )))
            }
        }
...
/// ```
pub fn atoi(data: &[u8]) -> Result<i32, YiiLianError> {
    let s = String::from_utf8_lossy(data);
    let rst = s.parse::<i32>().map_err(|e| YiiLianError::FrameParse(anyhow!(e)))?;
...
/// ```
pub fn bytes_to_sockaddr(bytes: &[u8]) -> Result<SocketAddr, YiiLianError> {
    ...
    match bytes.len() {
            ...
            let port_bytes_as_array: [u8; 2] =
                bytes[4..6]
                    ...
                    .map_err(|err: std::array::TryFromSliceError| {
                        YiiLianError::FrameParse(err.into())
                    })?;
        ...
        18 => Err(YiiLianError::FrameParse(anyhow!(
            "IPv6 is not yet implemented"
        ))),

        _ => Err(YiiLianError::FrameParse(anyhow!(
