...
- `pieces` 的值是一个字符串，它的长度是 20 的整数倍。它被分段为长度为 20 的若干字符串，每个分段字符串，都是其分段索引对应的文件分片的 SHA1 hash 值。
...
这就是握手帧，接下来是交替出现的**长度前缀 + 消息主体**的消息（帧）流。
...
> *译注：握手帧的格式为 `<pstrlen><pstr><reserved><info_hash><peer_id>`，可以参见下图*

[![图1 - handshake frame](../res/peer_wire_handshake.png)](../res/peer_wire_handshake.png)

### Peer message（消息帧）

所有非 keepalive 的消息都用 payload (载荷) 的第一个字节标识消息类型。
...
`choke`, `unchoke`, `interested`, 以及 `not interested` 消息时没有 payload 的。

**`bitfield`** 只作为第一条消息进行发送。它的 payload 是一个 bitfield (位图)，对于 downloader 已向对方发送过的分片，根据这些分片的索引在位图上设置为 1，其余的设置为 0 。Download 如果没有下载资源的任何数据，则跳过发送 `bitfield` 消息。位图的第一个字节从高到低对应索引 0 -7 ，第二个字节对应 8 - 15 ，以此类推。最后一个字节多余的位都设为 0 。

```
| len (4) | 5 (1) | bitfield |
```

**`have`** 消息的 payload 是一个数字，它是 downnloader 刚下载完并通过了 hash 校验的分片的索引值。

```
| len (4) | 4 (1) | index (4) |
```

**`request`** 消息包含一个分片索引，起始位置和长度。后两个是字节偏移量。长度，通常是 2 的幂次方，除非它是在文件末尾被截断的分片。目前所有的实现的分片长度都用 2 ^ 14 ( 16KiB )，如果请求的长度大于该值，则关闭连接。

```
| len (4) | 6 (1) | index (4) | offset (4) | length (4) |
```

**`cancel`** 消息有一个和 `request` 消息相同的 payload ，用于向 peer 取消之前发出的 request 请求。它们通常仅在下载结束时发送，这被称为 'endgame mode' （游戏结束模式）。当一个下载任务快结束时，最后几个分片从单点下载的话会花很长随时间。为了加快最后几片的下载速度，一旦 downloader 尚未完成下载的分片请求处于 pending 状态，它将向每一个可下载的 peer 发送请求来下载尚未完成的分片。为了防止这种情况变得特别低效，每次数据片到达时，downloader 都会向其它 Peer 发送 cancel。
...
**`piece`** 消息包含索引，起始位置和分片。注意，它们与请求消息有上下文关联。如果 `choke` 和 `unchoke` 消息快速连续地发送，或传输速度非常慢的话，可能会有未预期的数据片到达。
...
**`choke`** 使用该消息有几个原因。当同时通过多个连接发送数据时，TCP 拥塞控制的表现非常差。 此外，阻塞让每个对等点使用 `tit-for-tat-ish` (针锋相对) 算法来确保他们获得一致的下载速率。
