...
- **peer_id**
   向 tracker 发起请求的 downloader 的 id，它是一个长度为 20 的字符串。**Downloader 应在每次开始一个新的下载任务时重新随机生成一个新的 peer_id**，该值也几乎肯定要会被 (URL) 转码，实际实现时可通过前缀识别客户端（参见附录的 《Peer_id 客户端前缀》）。
...
```
| <len = 8> : int(4) | <msg_id = 4> : int(1) | <index> : int(4) |
```
