...
例如，如果`piece` 大小为 16KiB，则使用叶子的哈希值。如果使用的 `piece` 大小为 128KiB，则使用从叶子哈希值起算的第三层。仅覆盖文件末尾之外的数据（即仅用于平衡树）的层哈希值被省略。
所有哈希值都以其二进制格式存储。
...
> 因为 merkle 树是二叉树，叶子节点个数需要是 2 的幂次方，需要补充一个虚拟的叶子节点 HB4，它是 20 个值为 0 的字节数组, 然后分别计算这两组的 hash，
...
>   如果 `piece length` 是 128KiB，那么使用从叶节点往上数的第三层的哈希值。这意味着，每四个叶节点（因为 128KiB 是 16KiB 的 8 倍，但在树状结构中，
> 每上一层节点数量减半，所以这里是 4 倍关系）会被组合在一起，然后计算这个组合的哈希值。
