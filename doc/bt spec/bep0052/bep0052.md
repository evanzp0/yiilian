...
对于 `file tree` 项中每个大于 `piece length` 的文件，它包含一个字符串值。
它的 key 是 Merkle 根，而 values 由该默克尔树中某一层内的哈希值的连接组成。
...
> 也就是说第三层节点的每个 hash 值覆盖了一个 128KiB 的数据块，所以选择第三层节点做为 `piece layers` 。)
>
